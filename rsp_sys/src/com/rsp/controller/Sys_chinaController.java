...
public class Sys_chinaController {

	//自动装配
	@Autowired
	private Isys_chinaService isys_chinaService;
	...
	public JosnModel<Object> deleteWhereId(
			...
			try {
				...
				if(!StringUtils.isEmpty(id)){
					//执行查询
					List<Tab_sys_china> list=isys_chinaService.selectWhwerePid(id);
					if(null == list ||list.size()<=0){
						//执行删除
						int tt=isys_chinaService.deleteWhereId(id);
						if(tt>=1){
							josn.state=200;
							josn.msg="删除成功!";
						}else{
	...
	public JosnModel<Object> selectLike(
	    		@RequestParam(value="china_name",required=false)String china_name,
	    		HttpServletRequest request,
	    		HttpSession session){
			//实例化对象
			JosnModel<Object> josn=new JosnModel<Object>();
			...
			try {
				//验证非空
				if(!StringUtils.isEmpty(china_name)){
					//执行查询
					List<Tab_sys_china> list=isys_chinaService.selectLike(china_name);
					josn.data=list;
					josn.state=200;
					josn.msg="查询成功!";
				}else{
	...
	public JosnModel<Object> save(
    		@RequestParam(value="parent_id",required=false,defaultValue="0")String parent_id,
    		@RequestParam(value="china_name",required=false)String china_name,
    		HttpServletRequest request,
    		HttpSession session){
		//实例化对象
		JosnModel<Object> josn=new JosnModel<Object>();
		Tab_sys_china china=new Tab_sys_china();
		Tab_system_log sysLog=new Tab_system_log();
		//系统日志
		sysLog.setIp(GetIpUtil.getIpAddr(request));
		...
		try {
			...
			if(!StringUtils.isEmpty(parent_id)){
				if(!StringUtils.isEmpty(china_name)){
					//执行搜索
					List<Tab_sys_china> list=isys_chinaService.selectWhwerePid(parent_id);
					if(list!=null){
						//赋值
						china.setId(UUID.randomUUID().toString().replace("-", ""));
						china.setParent_id(parent_id);
						china.setChina_name(china_name);
						
						//执行保存
						int tt=isys_chinaService.save(china);
						if(tt>=1){
							josn.state=200;
							josn.msg="保存成功!";
							josn.data=china;
    ...
    public JosnModel<Object> selectWherePid(
		...
		try {
			...
			if(!StringUtils.isEmpty(pid)){
				//执行查询
				List<Tab_sys_china> list=isys_chinaService.selectWhwerePid(pid);
				if(list!=null){
					josn.state=200;
					josn.msg="请求成功!";
					josn.data=list;
