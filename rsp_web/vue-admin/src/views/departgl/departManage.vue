<template>
	<section>
		<el-form :inline="true" :model="filter">
			<el-form-item prop='name' label='科室名称'>
				<el-input v-model="filter.department_name" placeholder=""></el-input>
			</el-form-item>
	...
	<el-row class='my-form-item'>
		...
		:table="dataTable"  >
	<template slot-scope="props" slot="department_name">
        <a class="template-username" :rguid='props.obj.row.id' href='javascript:void(0);' @click='EditEach(props.obj.$index,props.obj.row)'>{{ props.obj.row.department_name }}</a>
	</template>
	...
	export default{
		data(){
			return{
				...
				dataTable:{
		            ...
		            tr:[
					{label:'科室属性',prop:'department_attribute',width:120
				},
					{label:'科室名称',prop:'department_name',width:120,show:'template'},
					{label:'电话',prop:'phone',width:120},
					{label:'负责人',prop:'responsible_perso',width:180},
		...
		mounted(){
			...
			getBmPersonDe(para).then((res) => {
					...
					this.department_attributeArr= [{id:'',coding_name:'全部'}].concat(res.data.list);
				});
		}	
	}
</script>
