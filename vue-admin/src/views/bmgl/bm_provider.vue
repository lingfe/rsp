<template>
	<section>
		...
		<el-form :inline="true" :model="filter">
			<el-form-item prop='manufacturer_name' label='生产厂家'>
				<el-input v-model="filter.manufacturer_name" placeholder=""></el-input>
			</el-form-item>
	...
	<template slot-scope="props" slot="manufacturer_name">
        <a class="template-username" :rguid='props.obj.row.id' href='javascript:void(0);' @click='EditEach(props.obj.$index,props.obj.row)'>{{ props.obj.row.manufacturer_name }}</a>
	</template>
	</my-el-table>
	<el-dialog  :title="calcTitle()" :visible.sync="dialogVisible" :close-on-click-modal='false'>
      <el-form ref="form" :model="form" label-width="80px" :rules='rules'>
        <el-form-item label="生产厂家" prop='manufacturer_name'>
...
//			               },
		            ...
		            tr:[
					{label:'编码名称',prop:'manufacturer_name',width:120,show:'template'
				},
				...
				rules:{
					manufacturer_name:[{required:true,message:'生产厂家名称不能为空',trigger:'blur'}]
				},
				form:{
					manufacturer_name:'',
					remark:''
		...
		methods:{
			...
			sure(){
				...
				if($this.type==0){
					...
					let para=Object.assign({hospital_id:''},$this.form);
				...
				$this.$confirm('确定这样修改').then(() => {
					$this.btnLoading=true;
					let para=Object.assign({id:$this.sels.id,hospital_id:''},$this.form);
			...
			EditEach(index,row){
				...
				$this.type=1;
				$this.dialogVisible=true;
				$this.form.manufacturer_name=row.manufacturer_name;
